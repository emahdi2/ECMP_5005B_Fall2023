...
of the book *"A Modern Approach to Regression with R"* written by Simon Sheather \url{https://gattonweb.uky.edu/faculty/sheather/book/data_sets.php}.
...
```{r}
nyc <- read.csv("https://gattonweb.uky.edu/faculty/sheather/book/docs/datasets/nyc.csv", header=TRUE)
attach(nyc)
...
This can be done by setting the argument $\color{blue}{\text{direction="backward"}}$ within the function $\color{blue}{\text{step()}}$ as follows:
...
The $\color{blue}{\text{step()}}$ function in __base__ __R__ performs the forward stepwise model, using an $\color{blue}{\text{AIC}}$ criterion, and setting the argument $\color{blue}{\text{direction="forward"}}$ within the function $\color{blue}{\text{step()}}$ as follows:
...
This approach is done by setting the argument $\color{blue}{\text{direction="both"}}$ within the function $\color{blue}{\text{step()}}$ as follows:
